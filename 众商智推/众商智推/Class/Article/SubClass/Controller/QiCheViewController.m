...
@interface QiCheViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,SDCycleScrollViewDelegate>
@property (strong, nonatomic) UITableView *qiCheTableView;
//开始请求的个数
...
@implementation QiCheViewController
...
- (void)viewDidLoad {
    ...
    [self.view addSubview:self.qiCheTableView];
    //加载轮播图
...
//        [self sendRequest];
        ...
        [self.qiCheTableView.header endRefreshing];
        [MBProgressHUD showSuccess:@"刷新成功"];
    }];
    self.qiCheTableView.header = gifHeader;
    //上拉提取
    ...
    self.qiCheTableView.footer = gifFooter;
}
#pragma mark -
#pragma mark -网络请求数据
- (void)sendRequest
{
    NSDictionary *parametDic = @{@"type":@(71),@"PageNo":@(self.startNum)};
    ...
    [AFNetworkUtil postDictWithUrl:BASE_STRURL parameters:parametDic successBackData:^(NSData *data) {
        ...
        //请求成功以后刷新表格
        [self.qiCheTableView reloadData];
        //请求成功，loading界面消失
        [LiangView removeLiangViewFromSuperView:self.view];
        
        //刷新操作
        if ([self.qiCheTableView.header isRefreshing]) {
            [self.qiCheTableView.header endRefreshing];
        }
        if ([self.qiCheTableView.footer isRefreshing]) {
            [self.qiCheTableView.footer endRefreshing];
        }
...
- (void)createScrollView
{
    ...
    self.qiCheTableView.tableHeaderView  = cycleScrollView;
...
#pragma 懒加载
- (UITableView *)qiCheTableView
{
    if (!_qiCheTableView) {
        _qiCheTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight- 120) style:UITableViewStylePlain];
        _qiCheTableView.delegate = self;
        _qiCheTableView.dataSource = self;
        _qiCheTableView.rowHeight = 110;
    }
    return _qiCheTableView;
}
