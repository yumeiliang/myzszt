...
@interface ZhiChangViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,SDCycleScrollViewDelegate>
@property (strong, nonatomic) UITableView *zhiChangTableView;
//开始请求的个数
...
@implementation ZhiChangViewController
...
- (void)viewDidLoad {
    ...
    [self.view addSubview:self.zhiChangTableView];
    //加载轮播图
...
//        [self sendRequest];
        ...
        [self.zhiChangTableView.header endRefreshing];
        [MBProgressHUD showSuccess:@"刷新成功"];
    }];
    self.zhiChangTableView.header = gifHeader;
    //上拉提取
    ...
    self.zhiChangTableView.footer = gifFooter;
}
#pragma mark -
#pragma mark -网络请求数据
- (void)sendRequest
{
    ...
    [AFNetworkUtil postDictWithUrl:BASE_STRURL parameters:parametDic successBackData:^(NSData *data) {
        ...
        //请求成功以后刷新表格
        [self.zhiChangTableView reloadData];
        //请求成功，loading界面消失
        [LiangView removeLiangViewFromSuperView:self.view];
        
        //刷新操作
        if ([self.zhiChangTableView.header isRefreshing]) {
            [self.zhiChangTableView.header endRefreshing];
        }
        if ([self.zhiChangTableView.footer isRefreshing]) {
            [self.zhiChangTableView.footer endRefreshing];
        }
...
- (void)createScrollView
{
    ...
    self.zhiChangTableView.tableHeaderView  = cycleScrollView;
...
#pragma 懒加载
- (UITableView *)zhiChangTableView
{
    if (!_zhiChangTableView) {
        _zhiChangTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight- 120) style:UITableViewStylePlain];
        _zhiChangTableView.delegate = self;
        _zhiChangTableView.dataSource = self;
        _zhiChangTableView.rowHeight = 110;
    }
    return _zhiChangTableView;
}
