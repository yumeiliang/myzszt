...
@interface XueBaViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,SDCycleScrollViewDelegate>
@property (strong, nonatomic) UITableView *xueBaTableView;
//开始请求的个数
...
- (void)viewDidLoad {
    ...
    [self.view addSubview:self.xueBaTableView];
    //加载轮播图
...
//        [self sendRequest];
        ...
        [self.xueBaTableView.header endRefreshing];
        [MBProgressHUD showSuccess:@"刷新成功"];
    }];
    self.xueBaTableView.header = gifHeader;
    //上拉提取
    ...
    self.xueBaTableView.footer = gifFooter;
}
#pragma mark -
#pragma mark -网络请求数据
- (void)sendRequest
{
    ...
    NSDictionary *parametDic = @{@"type":@(42),@"PageNo":@(self.startNum)};
    ...
    [AFNetworkUtil postDictWithUrl:BASE_STRURL parameters:parametDic successBackData:^(NSData *data) {
        ...
        //请求成功以后刷新表格
        [self.xueBaTableView reloadData];
        //请求成功，loading界面消失
        [LiangView removeLiangViewFromSuperView:self.view];
        
        //刷新操作
        if ([self.xueBaTableView.header isRefreshing]) {
            [self.xueBaTableView.header endRefreshing];
        }
        if ([self.xueBaTableView.footer isRefreshing]) {
            [self.xueBaTableView.footer endRefreshing];
        }
...
- (void)createScrollView
{
    ...
    self.xueBaTableView.tableHeaderView  = cycleScrollView;
...
#pragma 懒加载
- (UITableView *)xueBaTableView
{
    if (!_xueBaTableView) {
        _xueBaTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight- 120) style:UITableViewStylePlain];
        _xueBaTableView.delegate = self;
        _xueBaTableView.dataSource = self;
        _xueBaTableView.rowHeight = 110;
    }
    return _xueBaTableView;
}
