...
@interface LvXingViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,SDCycleScrollViewDelegate>
...
@implementation LvXingViewController
...
- (void)viewDidLoad {
    ...
    [self.view addSubview:self.lvXingTableView];
    //加载轮播图
...
//        [self sendRequest];
        ...
        [self.lvXingTableView.header endRefreshing];
        [MBProgressHUD showSuccess:@"刷新成功"];
    }];
    self.lvXingTableView.header = gifHeader;
    //上拉提取
    ...
    self.lvXingTableView.footer = gifFooter;
}
#pragma mark -
#pragma mark -网络请求数据
- (void)sendRequest
{
    NSDictionary *parametDic = @{@"type":@(41),@"PageNo":@(self.startNum)};
    ...
    [AFNetworkUtil postDictWithUrl:BASE_STRURL parameters:parametDic successBackData:^(NSData *data) {
        ...
        //请求成功以后刷新表格
        [self.lvXingTableView reloadData];
        //请求成功，loading界面消失
        [LiangView removeLiangViewFromSuperView:self.view];
        
        //刷新操作
        if ([self.lvXingTableView.header isRefreshing]) {
            [self.lvXingTableView.header endRefreshing];
        }
        if ([self.lvXingTableView.footer isRefreshing]) {
            [self.lvXingTableView.footer endRefreshing];
        }
...
- (void)createScrollView
{
    ...
    self.lvXingTableView.tableHeaderView  = cycleScrollView;
...
#pragma 懒加载
- (UITableView *)lvXingTableView
{
    if (!_lvXingTableView) {
        _lvXingTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight- 120) style:UITableViewStylePlain];
        _lvXingTableView.delegate = self;
        _lvXingTableView.dataSource = self;
        _lvXingTableView.rowHeight = 110;
    }
    return _lvXingTableView;
}
