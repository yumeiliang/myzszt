...
@interface DianZanViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,SDCycleScrollViewDelegate>
@property (strong, nonatomic) UITableView *dianZanTableView;
//开始请求的个数
...
//        [self sendRequest];
        ...
        [self.dianZanTableView.header endRefreshing];
        [MBProgressHUD showSuccess:@"刷新成功"];
    }];
    self.dianZanTableView.header = gifHeader;
    //上拉提取
    ...
    self.dianZanTableView.footer = gifFooter;
}
#pragma mark -
#pragma mark -网络请求数据
- (void)sendRequest
{
    NSDictionary *parametDic = @{@"type":@(34),@"PageNo":@(self.startNum)};
    ...
    [AFNetworkUtil postDictWithUrl:BASE_STRURL parameters:parametDic successBackData:^(NSData *data) {
        ...
        //刷新操作
        if ([self.dianZanTableView.header isRefreshing]) {
            [self.dianZanTableView.header endRefreshing];
        }
        if ([self.dianZanTableView.footer isRefreshing]) {
            [self.dianZanTableView.footer endRefreshing];
        }
...
- (void)createScrollView
{
    ...
    self.dianZanTableView.tableHeaderView  = cycleScrollView;
...
{
    if (!_dianZanTableView) {
        _dianZanTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight- 120) style:UITableViewStylePlain];
        _dianZanTableView.delegate = self;
        _dianZanTableView.dataSource = self;
        _dianZanTableView.rowHeight = 110;
    }
    return _dianZanTableView;
}
