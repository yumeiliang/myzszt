...
@interface CaiJingViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,SDCycleScrollViewDelegate>
@property (strong, nonatomic) UITableView *caiJingTableView;
//开始请求的个数
...
@implementation CaiJingViewController
...
- (void)viewDidLoad {
    ...
    [self.view addSubview:self.caiJingTableView];
    //加载轮播图
...
//        [self sendRequest];
        ...
        [self.caiJingTableView.header endRefreshing];
        [MBProgressHUD showSuccess:@"刷新成功"];
    }];
    self.caiJingTableView.header = gifHeader;
    //上拉提取
    ...
    self.caiJingTableView.footer = gifFooter;
}
#pragma mark -
#pragma mark -网络请求数据
- (void)sendRequest
{
    NSDictionary *parametDic = @{@"type":@(40),@"PageNo":@(self.startNum)};
    ...
    [AFNetworkUtil postDictWithUrl:BASE_STRURL parameters:parametDic successBackData:^(NSData *data) {
        ...
        //请求成功以后刷新表格
        [self.caiJingTableView reloadData];
        //请求成功，loading界面消失
        [LiangView removeLiangViewFromSuperView:self.view];
        
        //刷新操作
        if ([self.caiJingTableView.header isRefreshing]) {
            [self.caiJingTableView.header endRefreshing];
        }
        if ([self.caiJingTableView.footer isRefreshing]) {
            [self.caiJingTableView.footer endRefreshing];
        }
...
- (void)createScrollView
{
    ...
    self.caiJingTableView.tableHeaderView  = cycleScrollView;
...
#pragma 懒加载
- (UITableView *)caiJingTableView
{
    if (!_caiJingTableView) {
        _caiJingTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight- 120) style:UITableViewStylePlain];
        _caiJingTableView.delegate = self;
        _caiJingTableView.dataSource = self;
        _caiJingTableView.rowHeight = 110;
    }
    return _caiJingTableView;
}
