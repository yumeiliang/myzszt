...
@interface ChaoRenViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,SDCycleScrollViewDelegate>
@property (strong, nonatomic) UITableView *chaoRenTableView;
//开始请求的个数
...
@implementation ChaoRenViewController
...
- (void)viewDidLoad {
    ...
    [self.view addSubview:self.chaoRenTableView];
    //加载轮播图
...
//        [self sendRequest];
        ...
        [self.chaoRenTableView.header endRefreshing];
        [MBProgressHUD showSuccess:@"刷新成功"];
    }];
    self.chaoRenTableView.header = gifHeader;
    //上拉提取
    ...
    self.chaoRenTableView.footer = gifFooter;
}
#pragma mark -
#pragma mark -网络请求数据
- (void)sendRequest
{
    NSDictionary *parametDic = @{@"type":@(39),@"PageNo":@(self.startNum)};
    ...
    [AFNetworkUtil postDictWithUrl:BASE_STRURL parameters:parametDic successBackData:^(NSData *data) {
        ...
        //请求成功以后刷新表格
        [self.chaoRenTableView reloadData];
        //请求成功，loading界面消失
        [LiangView removeLiangViewFromSuperView:self.view];
        
        //刷新操作
        if ([self.chaoRenTableView.header isRefreshing]) {
            [self.chaoRenTableView.header endRefreshing];
        }
        if ([self.chaoRenTableView.footer isRefreshing]) {
            [self.chaoRenTableView.footer endRefreshing];
        }
...
- (void)createScrollView
{
    ...
    self.chaoRenTableView.tableHeaderView  = cycleScrollView;
...
#pragma 懒加载
- (UITableView *)chaoRenTableView
{
    if (!_chaoRenTableView) {
        _chaoRenTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight- 120) style:UITableViewStylePlain];
        _chaoRenTableView.delegate = self;
        _chaoRenTableView.dataSource = self;
        _chaoRenTableView.rowHeight = 110;
    }
    return _chaoRenTableView;
}
