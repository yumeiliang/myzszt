...
@interface KeJiViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,SDCycleScrollViewDelegate>
@property (strong, nonatomic) UITableView *keJiTableView;
//开始请求的个数
...
- (void)viewDidLoad {
    ...
    [self.view addSubview:self.keJiTableView];
    //加载轮播图
...
//        [self sendRequest];
        ...
        [self.keJiTableView.header endRefreshing];
        [MBProgressHUD showSuccess:@"刷新成功"];
    }];
    self.keJiTableView.header = gifHeader;
    //上拉提取
    ...
    self.keJiTableView.footer = gifFooter;
}
#pragma mark -
#pragma mark -网络请求数据
- (void)sendRequest
{
    NSDictionary *parametDic = @{@"type":@(47),@"PageNo":@(self.startNum)};
    ...
    [AFNetworkUtil postDictWithUrl:BASE_STRURL parameters:parametDic successBackData:^(NSData *data) {
        ...
        //请求成功以后刷新表格
        [self.keJiTableView reloadData];
        //请求成功，loading界面消失
        [LiangView removeLiangViewFromSuperView:self.view];
        
        //刷新操作
        if ([self.keJiTableView.header isRefreshing]) {
            [self.keJiTableView.header endRefreshing];
        }
        if ([self.keJiTableView.footer isRefreshing]) {
            [self.keJiTableView.footer endRefreshing];
        }
...
- (void)createScrollView
{
    ...
    self.keJiTableView.tableHeaderView  = cycleScrollView;
...
#pragma 懒加载
- (UITableView *)keJiTableView
{
    if (!_keJiTableView) {
        _keJiTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight- 120) style:UITableViewStylePlain];
        _keJiTableView.delegate = self;
        _keJiTableView.dataSource = self;
        _keJiTableView.rowHeight = 110;
    }
    return _keJiTableView;
}
