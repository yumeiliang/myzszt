...
@interface TiYuViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,SDCycleScrollViewDelegate>
@property (strong, nonatomic) UITableView *tiYuTableView;
//开始请求的个数
...
- (void)viewDidLoad {
    ...
    [self.view addSubview:self.tiYuTableView];
    //加载轮播图
...
//        [self sendRequest];
        ...
        [self.tiYuTableView.header endRefreshing];
        [MBProgressHUD showSuccess:@"刷新成功"];
    }];
    self.tiYuTableView.header = gifHeader;
    //上拉提取
    ...
    self.tiYuTableView.footer = gifFooter;
}
#pragma mark -
#pragma mark -网络请求数据
- (void)sendRequest
{
    NSDictionary *parametDic = @{@"type":@(75),@"PageNo":@(self.startNum)};
    ...
    [AFNetworkUtil postDictWithUrl:BASE_STRURL parameters:parametDic successBackData:^(NSData *data) {
        ...
        //请求成功以后刷新表格
        [self.tiYuTableView reloadData];
        //请求成功，loading界面消失
        [LiangView removeLiangViewFromSuperView:self.view];
        
        //刷新操作
        if ([self.tiYuTableView.header isRefreshing]) {
            [self.tiYuTableView.header endRefreshing];
        }
        if ([self.tiYuTableView.footer isRefreshing]) {
            [self.tiYuTableView.footer endRefreshing];
        }
...
- (void)createScrollView
{
    ...
    self.tiYuTableView.tableHeaderView  = cycleScrollView;
...
#pragma 懒加载
- (UITableView *)tiYuTableView
{
    if (!_tiYuTableView) {
        _tiYuTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight- 120) style:UITableViewStylePlain];
        _tiYuTableView.delegate = self;
        _tiYuTableView.dataSource = self;
        _tiYuTableView.rowHeight = 110;
    }
    return _tiYuTableView;
}
