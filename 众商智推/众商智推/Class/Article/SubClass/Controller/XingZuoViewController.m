...
@interface XingZuoViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,SDCycleScrollViewDelegate>

@property (strong, nonatomic) UITableView *xingZuoTableView;
//开始请求的个数
...
- (void)viewDidLoad {
    ...
    [self.view addSubview:self.xingZuoTableView];
    //加载轮播图
...
//        [self sendRequest];
        ...
        [self.xingZuoTableView.header endRefreshing];
        [MBProgressHUD showSuccess:@"刷新成功"];
    }];
    self.xingZuoTableView.header = gifHeader;
    //上拉提取
    ...
    self.xingZuoTableView.footer = gifFooter;
}
#pragma mark -
#pragma mark -网络请求数据
- (void)sendRequest
{
    NSDictionary *parametDic = @{@"type":@(55),@"PageNo":@(self.startNum)};
    ...
    [AFNetworkUtil postDictWithUrl:BASE_STRURL parameters:parametDic successBackData:^(NSData *data) {
        ...
        //请求成功以后刷新表格
        [self.xingZuoTableView reloadData];
        //请求成功，loading界面消失
        [LiangView removeLiangViewFromSuperView:self.view];
        
        //刷新操作
        if ([self.xingZuoTableView.header isRefreshing]) {
            [self.xingZuoTableView.header endRefreshing];
        }
        if ([self.xingZuoTableView.footer isRefreshing]) {
            [self.xingZuoTableView.footer endRefreshing];
        }
...
- (void)createScrollView
{
    ...
    self.xingZuoTableView.tableHeaderView  = cycleScrollView;
...
#pragma 懒加载
- (UITableView *)xingZuoTableView
{
    if (!_xingZuoTableView) {
        _xingZuoTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight- 120) style:UITableViewStylePlain];
        _xingZuoTableView.delegate = self;
        _xingZuoTableView.dataSource = self;
        _xingZuoTableView.rowHeight = 110;
    }
    return _xingZuoTableView;
}
