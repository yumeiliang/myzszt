...
@interface YangShengViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,SDCycleScrollViewDelegate>
@property (strong, nonatomic) UITableView *yangShengTableView;
//开始请求的个数
...
@implementation YangShengViewController
...
- (void)viewDidLoad {
   ...
   [self.view setBackgroundColor:ZSRandomColor];
    ...
    [self.view addSubview:self.yangShengTableView];
    //加载轮播图
...
//        [self sendRequest];
        ...
        [self.yangShengTableView.header endRefreshing];
        [MBProgressHUD showSuccess:@"刷新成功"];
    }];
    self.yangShengTableView.header = gifHeader;
    //上拉提取
    ...
    self.yangShengTableView.footer = gifFooter;
}
#pragma mark -
#pragma mark -网络请求数据
- (void)sendRequest
{
    NSDictionary *parametDic = @{@"type":@(35),@"PageNo":@(self.startNum)};
    ...
    [AFNetworkUtil postDictWithUrl:BASE_STRURL parameters:parametDic successBackData:^(NSData *data) {
        ...
        ZSLog(@"%ld",(unsigned long)self.allArray.count);
        
        //请求成功以后刷新表格
        [self.yangShengTableView reloadData];
        //请求成功，loading界面消失
        [LiangView removeLiangViewFromSuperView:self.view];
        
        //刷新操作
        if ([self.yangShengTableView.header isRefreshing]) {
            [self.yangShengTableView.header endRefreshing];
        }
        if ([self.yangShengTableView.footer isRefreshing]) {
            [self.yangShengTableView.footer endRefreshing];
        }
...
- (void)createScrollView
{
    ...
    self.yangShengTableView.tableHeaderView  = cycleScrollView;
...
#pragma 懒加载
- (UITableView *)yangShengTableView
{
    if (!_yangShengTableView) {
        _yangShengTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight- 120) style:UITableViewStylePlain];
        _yangShengTableView.delegate = self;
        _yangShengTableView.dataSource = self;
        _yangShengTableView.rowHeight = 110;
    }
    return _yangShengTableView;
}
