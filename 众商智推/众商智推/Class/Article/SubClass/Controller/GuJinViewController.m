...
@interface GuJinViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,SDCycleScrollViewDelegate>
@property (strong, nonatomic) UITableView *guJinTableView;
//开始请求的个数
...
@implementation GuJinViewController
...
- (void)viewDidLoad {
    ...
    [self.view addSubview:self.guJinTableView];
    //加载轮播图
...
//        [self sendRequest];
        ...
        [self.guJinTableView.header endRefreshing];
        [MBProgressHUD showSuccess:@"刷新成功"];
    }];
    self.guJinTableView.header = gifHeader;
    //上拉提取
    ...
    self.guJinTableView.footer = gifFooter;
}
#pragma mark -
#pragma mark -网络请求数据
- (void)sendRequest
{
    NSDictionary *parametDic = @{@"type":@(48),@"PageNo":@(self.startNum)};
    ...
    [AFNetworkUtil postDictWithUrl:BASE_STRURL parameters:parametDic successBackData:^(NSData *data) {
        ...
        //请求成功以后刷新表格
        [self.guJinTableView reloadData];
        //请求成功，loading界面消失
        [LiangView removeLiangViewFromSuperView:self.view];
        
        //刷新操作
        if ([self.guJinTableView.header isRefreshing]) {
            [self.guJinTableView.header endRefreshing];
        }
        if ([self.guJinTableView.footer isRefreshing]) {
            [self.guJinTableView.footer endRefreshing];
        }
...
- (void)createScrollView
{
    ...
    self.guJinTableView.tableHeaderView  = cycleScrollView;
...
#pragma 懒加载
- (UITableView *)guJinTableView
{
    if (!_guJinTableView) {
        _guJinTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight- 120) style:UITableViewStylePlain];
        _guJinTableView.delegate = self;
        _guJinTableView.dataSource = self;
        _guJinTableView.rowHeight = 110;
    }
    return _guJinTableView;
}
