...
@interface ShengHuoViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,SDCycleScrollViewDelegate>
@property (strong, nonatomic) UITableView *shengHuoTableView;
//开始请求的个数
...
//        [self sendRequest];
    ...
    self.shengHuoTableView.header = gifHeader;
    //上拉提取
    ...
    self.shengHuoTableView.footer = gifFooter;
}
#pragma mark -
#pragma mark -网络请求数据
- (void)sendRequest
{
    NSDictionary *parametDic = @{@"type":@(49),@"PageNo":@(self.startNum)};
    ...
    [AFNetworkUtil postDictWithUrl:BASE_STRURL parameters:parametDic successBackData:^(NSData *data) {
        ...
        //刷新操作
        if ([self.shengHuoTableView.header isRefreshing]) {
            [self.shengHuoTableView.header endRefreshing];
        }
        if ([self.shengHuoTableView.footer isRefreshing]) {
...
- (void)createScrollView
{
    ...
    self.shengHuoTableView.tableHeaderView  = cycleScrollView;
...
#pragma 懒加载
- (UITableView *)shengHuoTableView
{
    if (!_shengHuoTableView) {
        _shengHuoTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight- 120) style:UITableViewStylePlain];
        _shengHuoTableView.delegate = self;
        _shengHuoTableView.dataSource = self;
        _shengHuoTableView.rowHeight = 110;
    }
    return _shengHuoTableView;
}
