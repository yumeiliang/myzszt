...
@interface LaMaViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,SDCycleScrollViewDelegate>
@property (strong, nonatomic) UITableView *laMaTableView;
//开始请求的个数
...
@implementation LaMaViewController
...
- (void)viewDidLoad {
    ...
    [self.view addSubview:self.laMaTableView];
    //加载轮播图
...
//        [self sendRequest];
        ...
        [self.laMaTableView.header endRefreshing];
        [MBProgressHUD showSuccess:@"刷新成功"];
    }];
    self.laMaTableView.header = gifHeader;
    //上拉提取
    ...
    self.laMaTableView.footer = gifFooter;
}
#pragma mark -
#pragma mark -网络请求数据
- (void)sendRequest
{
    NSDictionary *parametDic = @{@"type":@(50),@"PageNo":@(self.startNum)};
    ...
    [AFNetworkUtil postDictWithUrl:BASE_STRURL parameters:parametDic successBackData:^(NSData *data) {
        ...
        //请求成功以后刷新表格
        [self.laMaTableView reloadData];
        //请求成功，loading界面消失
        [LiangView removeLiangViewFromSuperView:self.view];
        
        //刷新操作
        if ([self.laMaTableView.header isRefreshing]) {
            [self.laMaTableView.header endRefreshing];
        }
        if ([self.laMaTableView.footer isRefreshing]) {
            [self.laMaTableView.footer endRefreshing];
        }
...
- (void)createScrollView
{
    ...
    self.laMaTableView.tableHeaderView  = cycleScrollView;
...
#pragma 懒加载
- (UITableView *)laMaTableView
{
    if (!_laMaTableView) {
        _laMaTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight- 120) style:UITableViewStylePlain];
        _laMaTableView.delegate = self;
        _laMaTableView.dataSource = self;
        _laMaTableView.rowHeight = 110;
    }
    return _laMaTableView;
}
