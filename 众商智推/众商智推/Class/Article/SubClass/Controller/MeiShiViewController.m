...
@interface MeiShiViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,SDCycleScrollViewDelegate>
@property (strong, nonatomic) UITableView *meiShiTableView;
//开始请求的个数
...
- (void)viewDidLoad {
    ...
    [self.view addSubview:self.meiShiTableView];
    //加载轮播图
...
//        [self sendRequest];
        ...
        [self.meiShiTableView.header endRefreshing];
        [MBProgressHUD showSuccess:@"刷新成功"];
    }];
    self.meiShiTableView.header = gifHeader;
    //上拉提取
    ...
    self.meiShiTableView.footer = gifFooter;
}
#pragma mark -
#pragma mark -网络请求数据
- (void)sendRequest
{
    NSDictionary *parametDic = @{@"type":@(36),@"PageNo":@(self.startNum)};
    ...
    [AFNetworkUtil postDictWithUrl:BASE_STRURL parameters:parametDic successBackData:^(NSData *data) {
        ...
        //请求成功以后刷新表格
        [self.meiShiTableView reloadData];
        //请求成功，loading界面消失
        [LiangView removeLiangViewFromSuperView:self.view];
        
        //刷新操作
        if ([self.meiShiTableView.header isRefreshing]) {
            [self.meiShiTableView.header endRefreshing];
        }
        if ([self.meiShiTableView.footer isRefreshing]) {
            [self.meiShiTableView.footer endRefreshing];
        }
...
- (void)createScrollView
{
    ...
    self.meiShiTableView.tableHeaderView  = cycleScrollView;
...
#pragma 懒加载
- (UITableView *)meiShiTableView
{
    if (!_meiShiTableView) {
        _meiShiTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight- 120) style:UITableViewStylePlain];
        _meiShiTableView.delegate = self;
        _meiShiTableView.dataSource = self;
        _meiShiTableView.rowHeight = 110;
    }
    return _meiShiTableView;
}
