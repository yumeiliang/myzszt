...
@interface SiFangViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,SDCycleScrollViewDelegate>
@property (strong, nonatomic) UITableView *siFangTableView;
//开始请求的个数
...
@implementation SiFangViewController
...
- (void)viewDidLoad {
    ...
    [self.view addSubview:self.siFangTableView];
    //加载轮播图
...
//        [self sendRequest];
        ...
        [self.siFangTableView.header endRefreshing];
        [MBProgressHUD showSuccess:@"刷新成功"];
    }];
    self.siFangTableView.header = gifHeader;
    //上拉提取
    ...
    self.siFangTableView.footer = gifFooter;
}
#pragma mark -
#pragma mark -网络请求数据
- (void)sendRequest
...
//    NSString *strUrl = [NSString stringWithFormat:@"http://192.168.0.112/weika/appArticle_queryAllArticle.action"];
    ...
    [AFNetworkUtil postDictWithUrl:BASE_STRURL parameters:parametDic successBackData:^(NSData *data) {
        ...
        //请求成功以后刷新表格
        [self.siFangTableView reloadData];
        //请求成功，loading界面消失
        [LiangView removeLiangViewFromSuperView:self.view];
        
        //刷新操作
        if ([self.siFangTableView.header isRefreshing]) {
            [self.siFangTableView.header endRefreshing];
        }
        if ([self.siFangTableView.footer isRefreshing]) {
            [self.siFangTableView.footer endRefreshing];
        }
...
- (void)createScrollView
{
    ...
    self.siFangTableView.tableHeaderView  = cycleScrollView;
...
#pragma 懒加载
- (UITableView *)siFangTableView
{
    if (!_siFangTableView) {
        _siFangTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight- 120) style:UITableViewStylePlain];
        _siFangTableView.delegate = self;
        _siFangTableView.dataSource = self;
        _siFangTableView.rowHeight = 110;
    }
    return _siFangTableView;
}
