...
@interface DuanZiViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,SDCycleScrollViewDelegate>

@property (strong, nonatomic) UITableView *duanZiTableView;
//开始请求的个数
...
- (void)viewDidLoad {
    ...
    [self.view addSubview:self.duanZiTableView];
    //加载轮播图
...
//        [self sendRequest];
        ...
        [self.duanZiTableView.header endRefreshing];
        [MBProgressHUD showSuccess:@"刷新成功"];
    }];
    self.duanZiTableView.header = gifHeader;
    //上拉提取
    ...
    self.duanZiTableView.footer = gifFooter;
}
#pragma mark -
#pragma mark -网络请求数据
- (void)sendRequest
...
//    NSString *strUrl = [NSString stringWithFormat:@"http://192.168.0.112/weika/appArticle_queryAllArticle.action"];
    ...
    NSDictionary *parametDic = @{@"type":@(52),@"PageNo":@(self.startNum)};
    ...
    [AFNetworkUtil postDictWithUrl:BASE_STRURL parameters:parametDic successBackData:^(NSData *data) {
        ...
        //请求成功以后刷新表格
        [self.duanZiTableView reloadData];
        //请求成功，loading界面消失
        [LiangView removeLiangViewFromSuperView:self.view];
        
        //刷新操作
        if ([self.duanZiTableView.header isRefreshing]) {
            [self.duanZiTableView.header endRefreshing];
        }
        if ([self.duanZiTableView.footer isRefreshing]) {
            [self.duanZiTableView.footer endRefreshing];
        }
...
- (void)createScrollView
{
    ...
    self.duanZiTableView.tableHeaderView  = cycleScrollView;
...
{
    if (!_duanZiTableView) {
        _duanZiTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight- 120) style:UITableViewStylePlain];
        _duanZiTableView.delegate = self;
        _duanZiTableView.dataSource = self;
        _duanZiTableView.rowHeight = 110;
    }
    return _duanZiTableView;
}
