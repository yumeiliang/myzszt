//
//  StatisticArticleListViewController.m
//  众商智推
//
...
@interface StatisticArticleListViewController ()<UITableViewDataSource,UITableViewDelegate>

//排行榜的表格
@property (strong, nonatomic) UITableView *STableView;
//浏览量数组
@property (strong, nonatomic) NSArray *clickNumArr;
...
@implementation StatisticArticleListViewController
...
-(void)addRrefreshController
{
    ...
    DIYMJRefreshBackGifFooter *gifFooter = [DIYMJRefreshBackGifFooter footerWithRefreshingBlock:^{
        self.startNum += 1;
        [self sendRequest];
        [MBProgressHUD showSuccess:@"提取成功"];
...
- (void)sendRequest
{
    ...
    [AFNetworkUtil postDictWithUrl:PAIHANGBANG_STRURL parameters:parametDic successBackData:^(NSData *data) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    StatisticTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (cell == nil) {
        cell = [[StatisticTableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:cellIdentifier cellWidth:ScreenWidth];
    }
