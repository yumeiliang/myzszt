...
#import "DIYMJRefreshGifHeader.h"

@implementation DIYMJRefreshGifHeader
...
- (void)prepare
{
    ...
    UIImage *normalImage = [self reSizeImage:[UIImage imageNamed:@"pullRefresh"]
                                     newSize:CGSizeMake(20, 70)];
    [idleImages addObject:normalImage];
    [self setImages:idleImages forState:MJRefreshStateIdle];
    
    // 设置即将刷新状态的动画图片（一松开就会刷新的状态）
    NSMutableArray *refreshingImages = [NSMutableArray array];
    UIImage *beginImage = [self reSizeImage:[UIImage imageNamed:@"loading"]
                                    newSize:CGSizeMake(20, 70)];
    [refreshingImages addObject:beginImage];
